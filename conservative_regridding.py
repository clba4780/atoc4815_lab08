...
ax.plot(lats_f, f_src, label='Fine (0.25°)', color = 'midnightblue', linewidth = 2.0)      # fine grid line
ax.step(np.append(tgt_edges[:-1], tgt_edges[-1]), 
        np.append(f_tgt, f_tgt[-1]), 
        where = 'post', color = 'maroon', linewidth = 2.0,
        label='Conservative (1°)') # coarse step plot
ax.set_xlabel('Latitude (°N)')
ax.set_ylabel('Precip (mm/day)')
ax.legend()
plt.tight_layout()
plt.show()
plt.savefig('conservative_regrid_storm_profile.png', dpi = 150, bbox_inches = 'tight')
print("conservative_regrid_storm_profile.png saved successfully")